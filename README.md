...
Chrome
  └─ browser-extension（MV3：R11 DOM 填写、材料上传、人工暂停）
```
...
材料生成完成后，可以安装 `browser-extension` 中的 Chrome MV3 扩展辅助填报中国版权保护中心 R11 页面。扩展只接收当前申请页面发出的短期任务数据，不读取 Cookie、密码或验证码；用户必须手动登录、完成验证码/实名验证、处理签章页，并在最终提交前人工复核。内测安装和构建方式见 [browser-extension/README.md](./browser-extension/README.md)。
...
按文件名顺序执行 `supabase/migrations/` 下的 SQL 文件，创建 Auth 用户关联的申请、著作权人、材料、生成任务、填报任务、事件、生成记录、源码包、索引和 RLS。Storage 中创建名为 `generated-documents` 的私有 Bucket。DOCX 由现有 Python 模板生成，正式 PDF 由 `services/docx-pdf-converter` 中的 LibreOffice 隔离服务从 DOCX 转换，避免两套排版引擎造成页眉、页码、行号和中文字体错乱。
...
- `filing_jobs` 和 `filing_events` 保存扩展填报状态；扩展是本地浏览器执行层，自动化终点为“填写并上传后暂停”，不会最终提交。
- 扩展默认绑定正式网页应用 `https://ipgen.top`；本地开发时显式设置 `SOFTREG_APP_ORIGIN=http://localhost:3000`，生产扩展只允许 HTTPS origin。
