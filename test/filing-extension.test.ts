...
function formFor(holders: CopyrightHolder[], developmentMethod: "independent" | "cooperative"): CopyrightFormData {
  return {
    ...
    technical_features: "人工智能软件、云计算软件、自定义技术说明。",
    copyright_holders: holders,
  };
}

function dateGridMarkup(year: number, month: number): string {
  ...
  const cells = Array.from({ length: 42 }, (_, index) => {
    ...
    return `<td data-date="${iso}"${otherMonth ? ' class="other-month"' : ""}><div><span>${day}</span></div></td>`;
  });
  return Array.from({ length: 6 }, (_, row) => `<tr>${cells.slice(row * 7, row * 7 + 7).join("")}</tr>`).join("");
}
...
function multiPageFixture(
  ...
  return `<!doctype html><html><body>
    ...
    <script>
      ...
      window.__softwareCategoryOptionClicks = 0;
      ...
      function dateField(label, id) {
        const septemberGrid = ${JSON.stringify(dateGridMarkup(2026, 9))};
        return '<div class="fillin_item"><h3>' + label + '</h3><div class="fillin_info"><div class="datePicker">'
          ...
          + '<div class="datepick-dateBox"><div class="datepicker-body"><table><tbody>' + septemberGrid + '</tbody></table></div></div>'
          + '</div></div></div>';
      }
      ...
      function softwareCategoryField() {
        return '<div class="fillin_item"><h3>软件分类</h3><div class="fillin_info"><div class="hd-select" data-name="software-category">'
          ...
          + ['APP', '嵌入式软件', '中间件', '操作系统'].map((item) => '<div class="hd-option" data-value="' + item + '">' + item + '</div>').join("")
      ...
      function wireControls() {
        document.querySelectorAll(".hd-select").forEach((control) => {
          ...
          control.querySelectorAll(".hd-option").forEach((option) => {
            option.onclick = () => {
              if (control.dataset.name === "software-category") window.__softwareCategoryOptionClicks += 1;
        ...
        document.querySelectorAll(".datePicker").forEach((picker) => {
          ...
          picker.querySelectorAll("td").forEach((cell) => cell.onclick = () => {
            ...
            window.__dateCellClicks.push(cell.getAttribute("data-date") || "");
            input.value = cell.getAttribute("data-date") || form.development_date;
      ...
      function renderFeatures() {
        ...
        document.getElementById("next-features").onclick = () => {
          window.__featureValues = {
            ...
            technicalTags: Array.from(document.querySelectorAll("[data-name='technical-features'] input:checked")).map((input) => input.value),
...
test("R11 multi-page SPA fills each page after the user chooses applicant identity", async () => {
  ...
  try {
    ...
    assert.deepEqual(values.features?.technicalTags, ["人工智能软件", "云计算软件"]);
    assert.equal(values.features?.technicalFeatures, "自定义技术说明。");
...
test("R11 starts the second page before its authenticated owner row finishes mounting", async () => {
  ...
  try {
    ...
    assert.equal(values.category, "APP");
